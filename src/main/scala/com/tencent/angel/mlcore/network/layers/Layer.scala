...
abstract class Layer(val name: String, val outputDim: Int)(implicit val graph: Graph) extends Serializable {
      ...
      if (gradCollection == null) {
        ...
        (gradCollection, grad) match {
          ...
          case (left: RBCompIntFloatMatrix, right: BlasFloatMatrix) =>
            left.iadd(MatrixUtils.blas2RBCompDense(right.asInstanceOf[BlasFloatMatrix],
              gradCollection.asInstanceOf[RBCompIntFloatMatrix].getSubDim))
          case (left: RBCompIntDoubleMatrix, right: BlasDoubleMatrix) =>
            left.iadd(MatrixUtils.blas2RBCompDense(right.asInstanceOf[BlasDoubleMatrix],
              gradCollection.asInstanceOf[RBCompIntDoubleMatrix].getSubDim))
  ...
  private[mlcore] def toJson: JField = {
